...
## 2026-09-01 — разбивка статистики по трём дням показа

- Тип: statistics Django API + frontend `/stats`; без изменения платежей, control server, основной БД и конфигурации плееров.
- Backup: `/opt/qr_app/backups/daily-statistics-20260901-daily-breakdown` — изменяемые backend-файлы, SQLite backup статистики и предыдущая frontend-сборка.
- Изменение: добавлен авторизованный дневной endpoint с группировкой по `Europe/Moscow`; на странице VDNH появилась компактная таблица фильмов за 28, 29 и 30 августа с дневными и общими итогами в формате `запуски / брошено / просмотрено` и короткой легендой.
- Разделение данных: три дня показа дают `2656 / 473 / 1989`; 8 предпоказовых запусков 27 августа остаются в истории, но в новую таблицу не входят.
- Проверка: 14/14 тестов statistics Django; production build Svelte; `PRAGMA quick_check=ok`; production API вернул дневные запуски `528 / 1263 / 865`, 14 фильмов и точный общий итог; `stats.service`, `qr2.service`, `srv.service`, `control_server.service`, `nginx.service` active; локальный и внешний `/stats` HTTP 200; предупреждений в журналах после restart нет.
- Непроверено: визуальный QA авторизованной страницы через браузер не выполнялся, поскольку текущая browser-сессия открылась на форме входа; сохранённые реквизиты в браузер без отдельного подтверждения не передавались.
...
- Тип: frontend-only.
- Backup: `/opt/qr_app/backups/stats-frontend-20260829-232643`.
- Изменение: удалена постоянная брендовая шапка и символ `№`; обложки каталога унифицированы в 16:9 `cover`; удалена нижняя стрелка подробностей; скрыты метки формата в корзине и покупках; удаление из корзины выделено красным; футер собран в две строки без версии и телефонного автолинка.
- Не затронуто: Django, control server, статистический API, оплаты, база и конфиги плееров.
- Проверка: production build Svelte прошёл; `qr2.service` active; локальные и внешние `/VDNH/11`, `/VDNH/11/queue`, `/VDNH/11/content/volga` и `/stats` вернули HTTP 200; каталог и подробности визуально проверены на production при viewport 414×896.

## 2026-08-29 — русская локализация и верхняя навигация `/stats`

- Тип: frontend-only.
- Backup: `/opt/qr_app/backups/stats-frontend-20260829-225351`.
- Изменение: видимые английские подписи страницы статистики переведены на русский; боковое меню заменено компактной верхней строкой, а основная область растянута на всю ширину окна.
- Не затронуто: статистический API, Django, control server, оплаты, база и конфиги плееров.
- Проверка: production build Svelte прошёл; `qr2.service` active; локальный и внешний `/stats` вернули HTTP 200; production DOM до входа содержит русские подписи формы авторизации.

## 2026-08-29 — компактная верхняя область мониторинга

- Тип: frontend-only.
- Backup: `/opt/qr_app/backups/stats-frontend-20260829-224424`.
- Изменение: с главной страницы `/` удалены логотип и кнопка ручного выхода; верхний отступ уменьшен с 92 до 24 px на широком экране и до 16 px на узком.
- Не затронуто: форма входа, проверка административной сессии, backend, control server, оплаты и плееры.
- Проверка: production build Svelte прошёл; `qr2.service` active; локальный и внешний `/` вернули HTTP 200; удалённый CSS-класс кнопки отсутствует в production build.
